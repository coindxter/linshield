...
echo   "=================================="
echo   "|     Brentwood HS Cypat Club    |"
echo   "|          Team Kraken           |"
echo   "|     Linux Hardening Script     |"
echo   "=================================="
echo   "      Base by Walnut HS Code      "
...
if [[ ! $(whoami) = root && ! $DRYRUN = true ]]; then
    echo "Please try again with root privileges or run " sudo -s source harden.sh " "
...
if [[ ${BASH_SOURCE[0]} == ${0} ]]; then
    echo "Run 'source harden.sh' or 'sudo -s source harden.sh' instead"
    exit 1
fi
...
PS3='Would you like to run harden at this time (1/2)? '
options=("y" "n")
select opt in "${options[@]}"
do
    case $opt in
        "y")
            echo "you have chose to run harden"
            harden
            break
            ;;
        "n")
