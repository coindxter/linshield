...
# Quarantine media files in /home directory
locate -0 -i --regex "^/home/.*\.(aac|avi|flac|flv|m4a|mkv|mov|mp3|mp4|mpeg|mpg|ogg|rmvb|wma|wmv)$" |
    grep -Ev '.config|.local|.cache|Wallpaper' |
    tee "$DATA/banned_files" |
    xargs -r0 sh -c "chattr -i -a \$(basename _) && chattr -i -a _ && mv -t $BACKUP/quarantine _" ||
    perror "Couldn't quarantine files"

# Find suspicious files
locate -0 -i --regex "\.(aac|avi|flac|flv|gif|jpeg|jpg|m4a|mkv|mov|mp3|mp4|mpeg|mpg|ogg|png|rmvb|wma|wmv)$" |
    grep -Ev '^(/usr|/var/lib)' |
    tee "$DATA/sus_files"
